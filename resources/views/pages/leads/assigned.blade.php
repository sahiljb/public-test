    ...
    <x-slot:footerFiles>
        ...
        <script>
            $(document).ready(function() {
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "render": function(data, type, row) {
                            // Conditionally set the profile image URL
                            var profilePath = row.profile === '' ? '{{ url("images/avatar.png") }}' : '{{ url("storage") }}/' + row.profile;

                            // Construct the content for the 'Staff' column
                            return '<div class="d-flex align-items-center">' +
                                        '<img src="' + profilePath + '" alt="Profile" class="rounded-circle me-2" width="40" height="40">' +
                                        '<div>' +
                                            '<div>' + row.userName + '</div>' +   // Display user name
                                            '<div class="text-muted small">' + row.userPhone + '</div>' + // Display user phone
                                            '<div class="text-muted small">' + row.userEmail + '</div>' + // Display user email
                                        '</div>' +
                                '</div>';
                        },
                        "title": "Staff"
                    ...
                    },
                    {
                        ...
                        "render": function(data, type, row) {
                            if (data === 'normal') {
                                return '<span class="badge badge-light-dark mb-2 me-4">Normal</span>';
                            } else if (data === 'moderate') {
                                return '<span class="badge badge-light-info mb-2 me-4">Moderate</span>';
                            } else if (data === 'hot') {
                                return '<span class="badge badge-light-danger mb-2 me-4">Hot</span>';
                            } else {
                                return '<span class="badge badge-light-secondary mb-2 me-4">Unknown</span>';
                            }
                        },
                        "title": "Priority"
                    },
                    {
                        ...
                        "render": function(data, type, row) {
                            ...
                            return '<a class="badge badge-light-primary text-start me-2 bs-tooltip action-edit" href="' + editUrl + '" data-toggle="tooltip" data-placement="top" title="Edit">' +
                                ...
                                '<a class="badge badge-light-danger text-start bs-tooltip action-delete" href="javascript:void(0);" data-id="' + row.id + '" data-toggle="tooltip" data-placement="top" title="Delete">' +
                ...
                "buttons": [{
                    text: 'Click to Assign Leads',
                    className: 'btn btn-primary',
                    action: function(e, dt, node, config) {
                        window.location.href = "{{ route('leads.assign-lead') }}"; 
                    }
                }],
                "order": [],
                "oLanguage": {
                ...
                $('#table-list').on('click', '.action-delete', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            var baseUrlDelete = "{{ route('leads.delete', ['id']) }}";
